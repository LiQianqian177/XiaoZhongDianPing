...
const couponPrice = function(coupon){
  /*
  if (!coupon) return props.packageDetail.price;
  if (coupon.type.indexOf('DISCOUNT') >= 0){
    return props.packageDetail.price * coupon.amount / 100;
  } else if (coupon.type.indexOf('TO_FIXED') >= 0) {
    return Math.min(props.packageDetail.price, coupon.amount);
  } else if (coupon.type.indexOf('FIXED') >= 0) {
    return Math.max(0, props.packageDetail.price - coupon.amount);
  } else {
    return props.packageDetail.price;
  }
  */
  return coupon ? Math.max(0, props.packageDetail.price - coupon.discount) : props.packageDetail.price;
};
  ...
  try {
    ...
    if (!orderId){
      const orderResponse = await axios.post('http://localhost:8080/api/orders/create', {
        userId: userId,
        groupbuyId: props.packageDetail.id,
      });
    ...
    for (let i = 0; i < response.data.length; i++) {
      try {
        ...
        coupons.value[coupons.value.length-1].userCouponId = response.data[i].userCouponId;
        // useResponse.data is like "Discount amount: 15.00"
        coupons.value[coupons.value.length-1].discount = useResponse.data.substring(useResponse.data.indexOf(':') + 1).trim();
    ...
    <div class="order-confirm-modal">
      ...
      <div class="package-info">
        ...
        <div class="package-price">
          <span>原价: ¥{{ packageDetail.price.toFixed(2) }}</span>
        </div>
      </div>
